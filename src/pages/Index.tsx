...
import { Plus, Upload, X, Play, Database, PanelLeftClose, PanelLeft, Clock } from 'lucide-react';
...
export default function Index() {
  ...
  useEffect(() => {
    listDatabases().then(dbs => {
      setDatabases(dbs);
      if (dbs.length > 0) switchDatabase(dbs[0]);
    });
  ...
  const switchDatabase = async (db: DatabaseInfo) => {
    ...
    setTabs(prev => {
      const existing = prev.find(t => t.dbId === db.id);
      if (existing) {
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">A browser-based SQLite client</p>
                </div>
                <div className="flex gap-3 justify-center">
                  <Button onClick={createNewDatabase}>
                    <Plus className="h-4 w-4 mr-2" /> New Database
                  </Button>
                  <Button variant="outline" onClick={uploadDatabase}>
                    <Upload className="h-4 w-4 mr-2" /> Open File
                  </Button>
                </div>
              </div>
            </div>
